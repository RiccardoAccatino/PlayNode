...
function buildNav(cfg) {
  ...
  cfg.nav.forEach((label,i) => {
    ...
    b.textContent = label;
    b.onclick = () => { document.querySelectorAll('.nav-btn').forEach(x=>x.classList.remove('active')); b.classList.add('active'); showPage(label); };
    nav.appendChild(b);
  });
}
...
function buildSidebar(cfg) {
  ...
  if (!cfg.sidebar) { sb.classList.add('hidden'); return; }
  ...
  cfg.sidebar.forEach((label,i) => {
    ...
    b.onclick = () => { document.querySelectorAll('.sb-btn').forEach(x=>x.classList.remove('active')); b.classList.add('active'); showPage(label); };
    sb.appendChild(b);
  });
}
...
function localeSettings(){return `<div class="pg-title">Impostazioni Locale</div><div class="pg-sub">Configurazione Bar Belvedere</div><div class="card" style="margin-bottom:12px"><div class="card-hd">Informazioni locale</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;font-size:12px"><div><div style="color:var(--txt3);margin-bottom:3px">Nome</div><div>Bar Belvedere</div></div><div><div style="color:var(--txt3);margin-bottom:3px">Tipo</div><div>Pubblico</div></div><div><div style="color:var(--txt3);margin-bottom:3px">Indirizzo</div><div>Via del Corso 42, Roma</div></div><div><div style="color:var(--txt3);margin-bottom:3px">ID Locale</div><div style="font-family:monospace;color:var(--acc2)">LOC-007</div></div></div></div><div class="card"><div class="card-hd">Accesso e sicurezza</div><div style="display:flex;flex-direction:column;gap:10px"><button class="act-btn" style="width:fit-content">Cambia password edge</button><button class="act-btn" style="width:fit-content">Rigenera token API</button><button class="danger-btn" style="width:fit-content">Disconnetti locale</button></div></div>`;}
