/**
 * file: dashboard.js
 * Gestisce l'interfaccia principale dopo il login, integrando il design originale
 * con i dati reali provenienti dal database (userData).
 */
...
export function renderDashboard(userData) {
    ...
    if (userData.role === 'AdminPiattaforma' || userData.role === 'AdminGioco') mappedRole = 'platform';

    // 2. CONFIGURAZIONE DEI MENU
    const ROLES = {
        player: {
    ...
    // 3. LO SCHELETRO PRINCIPALE DELL'APP
    const appHtml = `
      <div class="app">
        <!-- TOPBAR -->
        <div class="topbar">
    ...
    // 4. FUNZIONI PER COSTRUIRE IL MENU DINAMICO
    function buildNav() {
    ...
    /* =====================================================================
       PAGINE DELLE DASHBOARD (IL TUO DESIGN ORIGINALE CON DATI DINAMICI)
       ===================================================================== */

    function playerDashboard() {
        return `
      <div class="pg-title">La mia Dashboard</div>
      <div class="pg-sub">Benvenuto ${userData.name} — ecco il tuo riepilogo personale</div>
    ...
    function localeGames(){return `<div class="pg-title">Giochi del Locale</div><div class="pg-sub">Configurazione e gestione giochi installati</div><div class="card"><table class="tbl"><thead><tr><th>Gioco</th><th>ID</th><th>Sensori</th><th>Stato</th><th>Azioni</th></tr></thead><tbody>${[{ico:'⚽',name:'Calciobalilla Verde',id:'CB-001',sensors:3,ok:true},{ico:'⚽',name:'Calciobalilla Rosso',id:'CB-002',sensors:3,ok:true},{ico:'🎯',name:'Freccette Dx',id:'FR-001',sensors:1,ok:true},{ico:'🎱',name:'Biliardo',id:'BL-001',sensors:2,ok:false}].map(g=>`<tr><td><span style="font-size:14px">${g.ico}</span> ${g.name}</td><td style="font-family:monospace;font-size:11px;color:var(--acc2)">${g.id}</td><td>${g.sensors} attivi</td><td><span class="badge ${g.ok?'b-grn':'b-red'}">${g.ok?'ok':'errore'}</span></td><td><button class="act-btn">Config</button></td></tr>`).join('')}</tbody></table></div>`;}
    function localeDevices(){return `<div class="pg-title">Dispositivi Edge</div><div class="pg-sub">Stato hardware e connessione MQTT</div><div class="stats-row-3"><div class="scard"><div class="scard-lbl">Edge principale</div><div class="scard-val" style="font-size:16px;color:var(--grn)">Online</div><div class="scard-delta up">Raspberry Pi 4 · 12ms latency</div></div><div class="scard"><div class="scard-lbl">Broker MQTT</div><div class="scard-val" style="font-size:16px;color:var(--grn)">Connesso</div><div class="scard-delta neutral">4 topic attivi</div></div><div class="scard"><div class="scard-lbl">Messaggi/min</div><div class="scard-val">142</div><div class="scard-delta up">picco 18:00</div></div></div><div class="card"><div class="card-hd">Topic MQTT attivi</div>${['locale/bar-belvedere/calciobalilla/cb-001/goal','locale/bar-belvedere/freccette/fr-001/score','locale/bar-belvedere/biliardo/bl-001/pocket','locale/bar-belvedere/edge/status'].map(t=>`<div class="list-row"><div class="dot d-grn"></div><div style="font-family:monospace;font-size:11px;color:var(--acc2);flex:1">${t}</div></div>`).join('')}</div>`;}
    function localeStats(){return `<div class="pg-title">Statistiche Locale</div><div class="pg-sub">Analisi utilizzo — Locale</div><div class="stats-row"><div class="scard"><div class="scard-lbl">Partite questo mese</div><div class="scard-val">412</div><div class="scard-delta up">+28% vs mese scorso</div></div><div class="scard"><div class="scard-lbl">Gioco più usato</div><div class="scard-val" style="font-size:16px">⚽</div><div class="scard-delta neutral">Calciobalilla</div></div><div class="scard"><div class="scard-lbl">Ora di punta</div><div class="scard-val" style="font-size:18px">18:00</div><div class="scard-delta neutral">mer-ven</div></div><div class="scard"><div class="scard-lbl">Giocatori unici</div><div class="scard-val">87</div><div class="scard-delta up">questo mese</div></div></div><div class="card"><div class="card-hd">Utilizzo per gioco</div>${[{ico:'⚽',name:'Calciobalilla',pct:58},{ico:'🎯',name:'Freccette',pct:24},{ico:'🎱',name:'Biliardo',pct:18}].map(g=>`<div class="skill-row"><div class="skill-name">${g.ico} ${g.name}</div><div class="skill-bar"><div class="skill-fill" style="width:${g.pct}%;background:var(--acc)"></div></div><div class="skill-pct">${g.pct}%</div></div>`).join('')}</div>`;}
    function localeSettings(){return `<div class="pg-title">Impostazioni Locale</div><div class="pg-sub">Configurazione account</div><div class="card" style="margin-bottom:12px"><div class="card-hd">Informazioni locale</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;font-size:12px"><div><div style="color:var(--txt3);margin-bottom:3px">Nome utente</div><div>${userData.name}</div></div><div><div style="color:var(--txt3);margin-bottom:3px">Ruolo</div><div>${userData.role}</div></div></div></div><div class="card"><div class="card-hd">Accesso e sicurezza</div><div style="display:flex;flex-direction:column;gap:10px"><button class="act-btn" style="width:fit-content">Cambia password edge</button><button class="act-btn" style="width:fit-content">Rigenera token API</button><button class="danger-btn" style="width:fit-content">Disconnetti locale</button></div></div>`;}
    ...
    function platformUsers(){return `<div class="pg-title">Gestione Utenti</div><div class="pg-sub">Tutti gli utenti registrati sulla piattaforma</div><div style="display:flex;gap:8px;margin-bottom:12px"><button class="act-btn">+ Nuovo utente</button></div><div class="card"><table class="tbl"><thead><tr><th>Utente</th><th>Ruolo</th><th>Locale</th><th>Partite</th><th>Stato</th><th>Azioni</th></tr></thead><tbody>${[{name:'Mario Rossi',role:'Giocatore',locale:'Bar Belvedere',matches:74,active:true},{name:'Anna Bianchi',role:'Giocatore',locale:'Milano',matches:58,active:true},{name:'Giuseppe Verdi',role:'Admin Locale',locale:'Bar Belvedere',matches:0,active:true},{name:'Lucia Neri',role:'Giocatore',locale:'Torino',matches:12,active:false},{name:'Roberto Blu',role:'Admin Gioco',locale:'—',matches:0,active:true}].map(u=>`<tr><td><div style="font-weight:500;font-size:12px">${u.name}</div></td><td><span class="badge ${u.role==='Giocatore'?'b-blu':u.role==='Admin Locale'?'b-amb':'b-grn'}">${u.role}</span></td><td style="font-size:11px;color:var(--txt2)">${u.locale}</td><td style="font-size:12px">${u.matches}</td><td><span class="badge ${u.active?'b-grn':'b-red'}">${u.active?'attivo':'sospeso'}</span></td><td><button class="act-btn">Edit</button></td></tr>`).join('')}</tbody></table></div>`;}
    function platformLocali(){return `<div class="pg-title">Gestione Locali</div><div class="pg-sub">Tutti i locali registrati sulla piattaforma</div><div style="display:flex;gap:8px;margin-bottom:12px"><button class="act-btn">+ Nuovo locale</button></div><div class="card"><table class="tbl"><thead><tr><th>Nome</th><th>Tipo</th><th>Città</th><th>Giochi</th><th>Stato</th><th></th></tr></thead><tbody>${[{name:'Bar Belvedere',type:'Pubblico',city:'Roma',games:4,ok:true},{name:'Circolo Sportivo',type:'Pubblico',city:'Milano',games:6,ok:true},{name:'Sala Giochi',type:'Pubblico',city:'Torino',games:3,ok:true},{name:'Casa di Mario Rossi',type:'Privato',city:'Roma',games:2,ok:true},{name:'Bar Sport',type:'Pubblico',city:'Genova',games:2,ok:false}].map(l=>`<tr><td style="font-weight:500;font-size:12px">${l.name}</td><td><span class="badge ${l.type==='Pubblico'?'b-blu':'b-amb'}">${l.type}</span></td><td style="font-size:11px">${l.city}</td><td>${l.games}</td><td><span class="badge ${l.ok?'b-grn':'b-red'}">${l.ok?'online':'offline'}</span></td><td><button class="act-btn">Dettagli</button></td></tr>`).join('')}</tbody></table></div>`;}
    function platformGames(){return `<div class="pg-title">Tipi di Gioco</div><div class="pg-sub">Definizione giochi e configurazione sensori</div><div style="display:flex;gap:8px;margin-bottom:12px"><button class="act-btn">+ Nuovo tipo gioco</button></div><div class="card"><table class="tbl"><thead><tr><th>Gioco</th><th>Sensori def.</th><th>Installazioni</th><th>Partite totali</th><th></th></tr></thead><tbody>${[{ico:'⚽',name:'Calcioballica',sensors:3,inst:12,matches:847},{ico:'🎯',name:'Freccette',sensors:1,inst:5,matches:312},{ico:'🎱',name:'Biliardo',sensors:2,inst:4,matches:189},{ico:'🎳',name:'Bocce',sensors:2,inst:3,matches:98}].map(g=>`<tr><td><span style="font-size:14px">${g.ico}</span> <span style="font-weight:500;font-size:12px">${g.name}</span></td><td>${g.sensors}</td><td>${g.inst}</td><td>${g.matches}</td><td><button class="act-btn">Config sensori</button></td></tr>`).join('')}</tbody></table></div>`;}
    function platformTournaments(){return `<div class="pg-title">Gestione Tornei</div><div class="pg-sub">Crea e gestisci tornei multi-locale</div><div style="display:flex;gap:8px;margin-bottom:12px"><button class="act-btn">+ Nuovo torneo</button></div><div class="card"><table class="tbl"><thead><tr><th>Torneo</th><th>Gioco</th><th>Locali</th><th>Partecipanti</th><th>Stato</th><th></th></tr></thead><tbody>${[{name:'Calcioballica Primavera 2026',game:'⚽ Calcioballica',locali:5,players:14,status:'in corso'},{name:'Coppa Biliardo Milano',game:'🎱 Biliardo',locali:2,players:8,status:'in corso'},{name:'Campionato Freccette 2026',game:'🎯 Freccette',locali:3,players:0,status:'in arrivo'},{name:'Gran Torneo Bocce 2025',game:'🎳 Bocce',locali:4,players:16,status:'concluso'}].map(t=>`<tr><td style="font-weight:500;font-size:12px">${t.name}</td><td>${t.game}</td><td>${t.locali}</td><td>${t.players||'—'}</td><td><span class="badge ${t.status==='in corso'?'b-grn':t.status==='in arrivo'?'b-blu':'b-amb'}">${t.status}</span></td><td><button class="act-btn">Gestisci</button></td></tr>`).join('')}</tbody></table></div>`;}
    function platformMonitor(){return `<div class="pg-title">Monitor Sistema</div><div class="pg-sub">Performance e salute dei microservizi</div><div class="stats-row"><div class="scard"><div class="scard-lbl">CPU media</div><div class="scard-val">34%</div><div class="scard-delta up">normale</div></div><div class="scard"><div class="scard-lbl">RAM usata</div><div class="scard-val">2.1GB</div><div class="scard-delta neutral">su 8GB</div></div><div class="scard"><div class="scard-lbl">Req/min API</div><div class="scard-val">847</div><div class="scard-delta up">picco 18:00</div></div><div class="scard"><div class="scard-lbl">Msg MQTT/min</div><div class="scard-val">1.2k</div><div class="scard-delta neutral">stabile</div></div></div><div class="card"><div class="card-hd">Latenza API per endpoint</div>${[{ep:'POST /api/matches',ms:42,ok:true},{ep:'GET /api/stats/player',ms:78,ok:true},{ep:'GET /api/tournaments',ms:31,ok:true},{ep:'POST /api/edge/sync',ms:234,ok:false}].map(e=>`<div class="skill-row"><div class="skill-name" style="width:180px;font-family:monospace;font-size:10px;color:var(--acc2)">${e.ep}</div><div class="skill-bar"><div class="skill-fill" style="width:${Math.min(e.ms/300*100,100)}%;background:${e.ok?'var(--grn)':'var(--red)'}"></div></div><div class="skill-pct" style="width:40px">${e.ms}ms</div></div>`).join('')}</div>`;}
    function platformLogs(){return `<div class="pg-title">Log & Audit</div><div class="pg-sub">Registro eventi e accessi alla piattaforma</div><div class="card"><div class="card-hd">Ultimi eventi di sistema</div>${[{t:'14:52:01',type:'INFO',msg:'Match created: ID#4821 · locale LOC-007 · game CB-001',svc:'match-service'},{t:'14:51:48',type:'WARN',msg:'Edge LOC-004 offline — buffering 12 events locally',svc:'edge-sync'},{t:'14:49:22',type:'INFO',msg:'User login: mario.rossi@email.it · IP 192.168.1.42',svc:'auth-service'},{t:'14:47:10',type:'INFO',msg:'Goal event received: CB-001 team_a score=3',svc:'mqtt-broker'},{t:'14:45:00',type:'INFO',msg:'Tournament standings updated: T-003 Calciobalilla Primavera',svc:'tournament-service'},{t:'14:31:55',type:'ERROR',msg:'Edge sync timeout LOC-004 — retry in 30s',svc:'edge-sync'},{t:'14:30:00',type:'INFO',msg:'Stats aggregation job completed: 847 records',svc:'stats-service'}].map(e=>`<div class="list-row" style="gap:8px"><span style="font-size:9px;color:var(--txt3);width:56px;flex-shrink:0;font-family:monospace">${e.t}</span><span class="badge ${e.type==='INFO'?'b-blu':e.type==='WARN'?'b-amb':'b-red'}" style="width:36px;text-align:center">${e.type}</span><span style="font-family:monospace;font-size:10px;flex:1;color:var(--txt2)">${e.msg}</span><span style="font-size:9px;color:var(--txt3);flex-shrink:0">${e.svc}</span></div>`).join('')}</div></div>`;}
}