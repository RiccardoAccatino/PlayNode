...
export function playerDashboard() {
  ...
  // FIX: Ritardiamo la chiamata di 50ms per dare il tempo al Router 
  // di inserire effettivamente questo HTML nel DOM del browser.
  setTimeout(() => {
...
/**
 * Funzione asincrona che scarica i dati e inietta l'HTML.
 */
async function loadDashboardData(containerId) {
  try {
    const userId = localStorage.getItem('userId');

    // Controllo di sicurezza: se l'utente non è loggato
    if (!userId) {
      const container = document.getElementById(containerId);
      if (container) container.innerHTML = `<div style="color:var(--red); padding: 20px;">Errore: Utente non autenticato. Ritorna al login.</div>`;
      return;
    }
    ...
    // Se il container è ancora null significa che l'utente ha cambiato vista molto velocemente
    if (!container) {
      console.warn("Container non trovato. Il rendering della dashboard è stato interrotto.");
      return;
    }

    // 1. MAPPATURA STATISTICHE
    const s = stats || {
      partiteGiocate: 0,
      vittorie: 0,
      punteggioTotale: 0
    ...
    const container = document.getElementById(containerId);
    if (container) {
      container.innerHTML = `<div style="color:var(--red); padding: 20px; text-align:center;">Errore nel caricamento dei dati dal server.<br><small>${err.message}</small></div>`;
    }
  }
}
